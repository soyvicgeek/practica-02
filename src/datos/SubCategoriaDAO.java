...
public class SubCategoriaDAO implements CrudSimpleInterface<Categorias> {
    ...
    public List<SubCategorias> listarSubCategorias() {
        List<SubCategorias> registros = new ArrayList();
        String sql;
        try{
            sql="SELECT idSubCategoria, nombreSubCategoria FROM subcategorias order by nombreSubCategoria asc";
            ps=CON.conectar().prepareStatement(sql);
            rs=ps.executeQuery();
            while(rs.next()) {
                registros.add(new SubCategorias(rs.getInt(1),rs.getString(2)));
            }
            ps.close();
            rs.close();
        } catch(SQLException e) {
